...
compileMethodsInMap(J9VMThread * vmThread, ClassToMethodsMap &map)
   ...
   if (getenv("TR_LoadExtraClassesBeforeCompiling"))
      {
      char *classToLoad = "io/netty/util/concurrent/PromiseCombiner";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      J9Class *clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->applicationClassLoader, 0);
      if (clazz == NULL)
         {
         fprintf(stdout, "Failed to load %s\n", classToLoad);
         }
     
      classToLoad = "io/netty/util/concurrent/PromiseCombiner$1";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->applicationClassLoader, 0);
      if (clazz == NULL)
         {
         fprintf(stdout, "Failed to load %s\n", classToLoad);
         }

      classToLoad = "java/util/AbstractList$ListItr";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->systemClassLoader, 0);
      if (clazz == NULL)
         {
         fprintf(stdout, "Failed to load %s\n", classToLoad);
         }

      classToLoad = "java/util/concurrent/atomic/Striped64$Cell";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->systemClassLoader, 0);
      if (clazz == NULL)
         {
      ...
      classToLoad = "java/util/concurrent/ConcurrentLinkedQueue$Itr";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->systemClassLoader, 0);
      if (clazz == NULL)
         {
      ...
      classToLoad = "sun/nio/ch/IOVecWrapper";
      fprintf(stdout, "Loading class %s\n", classToLoad);
      clazz = vmFuncs->internalFindClassUTF8(vmThread, (U_8 *)classToLoad, strlen(classToLoad), javaVM->systemClassLoader, 0);
      if (clazz == NULL)
         {
